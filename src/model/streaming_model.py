...
class StreamingModel:
    ...
    def predict(self, X_num, X_artists, X_text):
        ...
        w_sum = sum(self.weights) + 1e-9
        w = [wi / w_sum for wi in self.weights]
    ...
    def learn(self, X_num, X_artists, X_text, y):
        ...
        for i, m in enumerate(self.models):
            ...
            self.weights[i] = 1.0 / (sum(self.errors[i]) / len(self.errors[i]) + 1e-6)
    ...
    def save_models(self, filepath):
        ...
        for i, (model, feat_cfg) in enumerate(zip(self.models, self.features_config)):
            ...
            if isinstance(optimizer, optim.FTRLProximal):
                ...
                self.logger.info(
                    f"Model {i:2d} | optimizer={type(optimizer).__name__} | "
                    f"features={feat_cfg}"
                )
